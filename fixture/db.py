...
class DbFixture:
    ...
    def get_contact_list(self):
        ...
        cursor = self.connection.cursor()
        try:
            cursor.execute("select id, firstname, middlename, lastname, nickname, title, company,"
                           "address, home, mobile, work, fax, email, email2, email3, homepage,"
                           "bday, bmonth, byear, aday, amonth, ayear from addressbook")
            for row in cursor:
                ...
                list.append(Contact(id=str(id), firstname=firstname, middlename=middlename,
                                    ...
                                    company=company, address=address, homephone=home,
                                    mobilephone=mobile, workphone=work, fax=fax, email=email,
                                    email2=email2, email3=email3, homepage=homepage, bday=bday,
                                    bmonth=bmonth, byear=byear, aday=aday, amonth=amonth,
                                    ayear=ayear))
        finally:
            cursor.close()
        return list

    def get_inside_group(self):
        list = []
        cursor = self.connection.cursor()
        try:
